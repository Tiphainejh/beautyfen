...
def generate_scss(data):
  ...
  d.append('// Ionicons Variables')
  d.append('// --------------------------\n')
  d.append('&euro;ionicons-font-path: "../fonts" !default;')
  d.append('&euro;ionicons-font-family: "%s" !default;' % (font_name) )
  d.append('&euro;ionicons-version: "%s" !default;' % (font_version) )
  d.append('&euro;ionicons-prefix: %s !default;' % (css_prefix) )
  d.append('')
  for ionicon in data['icons']:
    chr_code = ionicon['code'].replace('0x', '\\')
    d.append('&euro;ionicon-var-%s: "%s";' % (ionicon['name'], chr_code) )
  f = open(variables_file_path, 'w')
  f.write( '\n'.join(d) )
  f.close()
  ...
  group = [ '.%s' % (data['name'].lower()) ]
  for ionicon in data['icons']:
    group.append('.#{&euro;ionicons-prefix}%s:before' % (ionicon['name']) )
  ...
  for ionicon in data['icons']:
    chr_code = ionicon['code'].replace('0x', '\\')
    d.append('.#{&euro;ionicons-prefix}%s:before { content: &euro;ionicon-var-%s; }' % (ionicon['name'], ionicon['name']) )
